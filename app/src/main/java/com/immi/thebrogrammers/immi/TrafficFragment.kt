...
class TrafficFragment : Fragment() {
  override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
    ...
    val dataList = ImmIParser.getTrafficSubQIndices(cityName).toList()
    val data: List<ValueDataEntry> = dataList.map({ c -> ValueDataEntry(c.first, c.second.toInt()) })
    val anyChartView = view.findViewById<com.anychart.AnyChartView>(R.id.any_chart_view_traffic)
    ...
    barChart.title("Component of traffic level surveyed")
